...
def home_body(job, flash):
    ...
    for prefix, title in (("daily-", "일간"), ("weekly-", "주간")):
        items = "".join(f'<li><a href="/file/{e(n)}" target="_blank" rel="noopener">{e(page_label(n))}</a>'
                        f'{notes_link(n)}{preview_link(n)}</li>' for n in list_pages(prefix))
...
ACTIONS = {"/run": act_run, "/settings/source": act_source, "/settings/host/add": act_host_add,
           ...
           "/notes": act_notes, "/notes/followup": act_followup}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def route(self, method):
        ...
        if method == "POST":
            ...
            # (kind, message, where to go back to); a non-str third item is a confirm button (기록 삭제)
            if result and len(result) == 3 and isinstance(result[2], str):
