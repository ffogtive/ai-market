...
0. **실데이터 확인 (사용자 맥, 한 줄).** `retro update && retro && retro week && retro app` → 새 템플릿·요약 근거·"요청함" 표시, 회고 쓰기 저장→페이지 반영, 다음 날 확인이 맞는지. 요약 스키마가 바뀌어 첫 실행은 새로 요약(1~2분씩).
...
운영 메모: 사용자는 비개발자(CLAUDE.md). 사용자 기기 작업은 복붙 한 줄로. Codex 리뷰는 `retro/tools/codex_review.sh`를 GPU 서버에서(클라우드는 OpenAI 차단, 서버 Codex는 샌드박스가 안 돼 문서를 프롬프트에 넣음).
